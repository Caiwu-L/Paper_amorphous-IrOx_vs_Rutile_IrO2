...
function get_upper_bound(y_index_,switching_values,upper_set, y_array,pureZ_array)
    current_y=y_array[y_index_]
    upper_bound = fill(0.0, size(pureZ_array,2))
    i=size(switching_values)[1]
    if current_y<=switching_values[1]
        upper_bound[1] = upper_set[1]# limit the first redox's second increase
    
    elseif current_y<=switching_values[2]&&current_y>=switching_values[1]
        upper_bound[1:2].=upper_set[1:2] # limit the second redox's second increase

    elseif current_y>switching_values[2]
        upper_bound[1:3].=upper_set[1:3]  # limit the third redox's second increase
        
    end     
    println("upper bound array set to $upper_bound")
