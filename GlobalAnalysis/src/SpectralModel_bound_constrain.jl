...
include("DataImport_bound_new.jl")

#### INPUT

# set boundary and constraint for fitting  (y<first number, only one component, first<y< second,component 1 and 2, second <y< third number, three component ))
y_switching_values=[0.6,1.1,2]
...
dirData = raw"D:\OneDrive - Imperial College London\IC_research\IrOx\KIE effect\IrOx_KIE_SEC\Julia_analysis\H2O_deconvolute\test"
...
dirPureY = raw"D:\OneDrive - Imperial College London\IC_research\IrOx\KIE effect\IrOx_KIE_SEC\Julia_analysis\H2O_deconvolute\Pure_spectra_20mvs_2"
...
upper = fill(1, size(pureZ,2))
startParam = fill(0.001, size(pureZ,2))
upper_set = [0.06957,10,10] #upper bound to control maximum manually
...
Threads.@threads for ny in 1:length(y)
    ...
    if true # set the boundary for component to start and set the maximum of each component
        upper=get_upper_bound(ny,y_switching_values,upper_set,y,pureZ)
    end 
    ...
    #= limit the decay after reaching maximum （cannot limit the maximum and decay at the same time??）
    if ny>=2
        if paramOpt[ny,1]<paramOpt[ny-1,1]
                lower[1]=paramOpt[ny-1,1]
                startParam[1]=lower[1]+0.00001
        end
        if paramOpt[ny,2]<paramOpt[ny-1,2]
                lower[2]=paramOpt[ny-1,2]
                startParam[2]=lower[2]+00001
        end
    end
    =#

    #=
    #limit the decay after reaching maximum
    if ny>=2
        if paramOpt[ny,1]<paramOpt[ny-1,1]
        lower[1]=paramOpt[ny-1,1]
        startParam[1]=lower[1]+0.001
        end
        if paramOpt[ny,2]<paramOpt[ny-1,2]
        lower[2]=paramOpt[ny-1,2]
        startParam[2]=lower[2]+0.001
        end
    end
   =#
end
...
#write differential
paramOpt_1=[0 0 0;paramOpt]
paramOpt_2=paramOpt_1[1:end-1,:]
differential=paramOpt.-paramOpt_2
writedlm("differential.csv",[y differential],',')
######## PLOT FIT RESULT 
...
tickfont=fo1, legendfont=fo1, guidefont=fo2, 
...
# plot fit kinetics 
p2 = plot(yOffset[yBool], paramOpt[yBool,:],linewidth=1,xlabel="Potential (V vs RHE)", 
...
p3 = plot(yOffset[yBool], paramOptNorm[yBool,:],xlabel="Potential (V vs RHE)", #xlim=(-1,1.5),
    ylabel = "Normalised ΔA", title="Kinetics from spectral model", legend=false,
    markerstrokewidth=0, markersize=3.5)
# write normalise fit Kinetics
writedlm("GA_kinetics_norm.csv",[y paramOptNorm],',')
...
#write all fitting spectra
fitting_results=[x zSim]
writedlm("fittingspectra.csv",fitting_results,',')
#write residuals
zDiff = z .- zSim
residuals=[x zDiff]
writedlm("residual.csv",residuals,',')
