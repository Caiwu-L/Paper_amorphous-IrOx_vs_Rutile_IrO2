...
function get_upper_bound(y_index_,switching_values,y_array,pureZ_array)
    current_y=y_array[y_index_]
    upper_bound = fill(0.0, size(pureZ_array,2))
    for i=1:size(switching_values)[1]
        if current_y<=switching_values[1]
            upper_bound[1] =10
            break
        elseif current_y<=switching_values[i]&&current_y>=switching_values[i-1]
            upper_bound[1:i].=10
            
        elseif current_y>=switching_values[end]
            upper_bound[1:end].=10
