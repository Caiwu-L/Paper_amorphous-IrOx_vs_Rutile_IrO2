...
baseline_potential_AgAgCl =1.55; %in AgAgCl
Correct_baseline=0; %SET TO 0 IF NOT CORRECTING BASELINE
RHE_conv_factor=0;
iR=35;%input R obtain in EIS, put 0 if no need iR correct.
iR_compen=1; %input iR compensation percentage
smoothing_weight=100;
WL_max=850;
WL_min=440;
filename1='RuOx_decay_1.55_0.4_reverseSEC';
filename2='WL';
filename3='RuOx_decay_1.55_0.4_reverse_JV';
...
%(to match both forward scan and backward scan)
    if baseline_potential<potentials_array(end) %(means it is forward scan)
        iR_correct_region=(baseline_potential_AgAgCl<=potential&potential<=potentials_array(end));
    elseif reverse_plot==1%(backward scan, with reverse option)
        iR_correct_region= baseline_potential_AgAgCl<=potential
    else 
        iR_correct_region=(baseline_potential_AgAgCl>=potential&potential>=potentials_array(end));
    end
